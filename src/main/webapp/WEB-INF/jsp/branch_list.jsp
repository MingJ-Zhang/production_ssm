...
<table class="easyui-datagrid" id="branchList" title="机构列表" data-options="singleSelect:false,collapsible:true,
		pagination:true,rownumbers:true,url:'branch/list', ethod:'get',pageSize:20,
		fitColumns:true,toolbar:toolbar_branch">
    <thead>
        <tr>
...
<div  id="toolbar_branch" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_branch" style="float: right;">
        ...
        <div id="menu_branch" style="width:120px"> 
			...
			<div data-options="name:'branchName'">机构名称</div>
		</div>     
    </div>  

</div>  

<div id="branchEditWindow" class="easyui-window" title="编辑部门" 
	data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'branch/edit'" 
	style="width:65%;height:65%;padding:10px;">
</div>

<div id="branchAddWindow" class="easyui-window" title="添加部门" 
	data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'branch/add'" 
	style="width:65%;height:65%;padding:10px;">
</div>
...
function doSearch_branch(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#branchList").datagrid({
	        ...
	        toolbar:"toolbar_branch", url:'branch/list', method:'get', loadMsg:'数据加载中......', 
		...
		$("#branchList").datagrid({  
	        title:'机构列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, 
	        	method:'get', nowrap:true,  
	        toolbar:"toolbar_branch", url:'branch/search_branch_by_'+name+'?searchValue='+value, 
